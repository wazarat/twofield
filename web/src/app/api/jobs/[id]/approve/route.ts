...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    let updated = await approveJob(job, (body.note ?? "").trim().slice(0, 500));
    try {
      updated = await recordFeedback(updated, appBaseUrl(request));
    } catch (err) {
      console.error("feedback after approve failed", { jobId: id, message: err instanceof Error ? err.message.split("\n")[0] : String(err) });
    }
    ...
    console.error("approve failed", { jobId: id, message });
